...
# Function to clean salary strings (remove $, commas, spaces)
...
# Reshape from wide to long format (easier to merge with stats)
...
# Convert season format to match NBA API format (2022/2023 -> 2022-23)
salary_long['SEASON_ID'] = salary_long['Season'].apply(
    lambda x: x.split('/')[0] + '-' + x.split('/')[1][-2:]
)

# Clean up
salary_long = salary_long[salary_long['Salary'] > 0]  # Remove $0 salaries
